...
| [pdf-verify-mcp](https://github.com/shuji-bonji/pdf-verify-mcp)                         | MCP                 | pdf             | v0.15.2 | `shuji-bonji/pdf-verify-mcp`             |
...
- **pdf-trust (revision history)**: In `pdf-verify-mcp` **v0.14.2 and earlier**, a `/Prev` link that could not be followed was read as "there is no previous cross-reference section", so the revision chain was reported as complete when it was not — a specimen with 8 revisions and 5 signatures came back as having one (**fixed in v0.15.0**). Any audit whose report promises the *full* revision history — the `legal` and `medical` profiles, and any contract dispute over whether the body text was altered after signing — must be run on **v0.15.0 or later**. On earlier versions the shortened list cannot be distinguished from a document that was never changed.
- **pdf-trust (what a report can claim)**: two `pdf-verify-mcp` changes affect wording rather than verdicts. **v0.15.1** records which veraPDF build decided a conformance result (`authoritativeValidation.version`) — a rule count such as "146 / 146" is only comparable across runs of the same build, so a report from an earlier version cannot say what produced its numbers. **v0.15.2** corrects the `verify_integrity` description: a returned `revisions` list can be a part of the history rather than all of it, and the two cases that shorten it (a chain that ends before the original revision, and a last `startxref` that does not point at a parseable section) reach the caller through `notes` alone — there is no boolean, and no note says the chain WAS complete. Neither is a defect in the verdicts of earlier versions; both change what a report can honestly claim.
