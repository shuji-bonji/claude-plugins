...
- **Node.js requirement (as of 2026-09-06)**: the MCP servers start through `npx`, so your local Node.js version decides whether they start at all. `pdf-reader-mcp` / `pdf-verify-mcp` / `pdf-writer-mcp` / `pdf-spec-mcp` and `houki-nta-mcp` need **Node 20 or later**; `houki-egov-mcp` (since v0.4.0, 2026-09-07) / `rfcxml-mcp` / `w3c-mcp` / `web-compat-mcp` / `xcomet-mcp` / `epsg-mcp` / `rxjs-mcp` need **Node 22 or later**; `ifc-core-mcp` needs **Node 22.12 or later** (`engines.node` in each repo's `package.json`). Node 20 reached end of life on 2026-04-30, and the eight servers that require Node 22 do not start on Node 20.
- **Migration to MCP SDK v2**: starting with the four PDF family servers on 2026-08-27, by 2026-09-07 twelve MCP servers — every one except `houki-nta-mcp` — moved from `@modelcontextprotocol/sdk` (v1) to `@modelcontextprotocol/server@^2.0.0` (`houki-egov-mcp` moved in v0.4.0 on 2026-09-07; `houki-nta-mcp` stays on `@modelcontextprotocol/sdk@^1.29.0` and its migration is tracked in [claude-plugins#4](https://github.com/shuji-bonji/claude-plugins/issues/4)). Two changes reach callers of the eleven servers that use the SDK's `registerTool`. (1) **`inputSchema` in `tools/list` now declares `$schema` as JSON Schema 2020-12** (it was draft-07). (2) **An argument whose type does not match is rejected by the SDK before the handler runs and comes back as a tool result with `isError: true` (`Input validation error: ...`)** — until now each server validated on its own and the JSON it returned differed. Whether an argument the schema does not declare is rejected or silently dropped still differs per server (the four PDF family servers reject it — see the next paragraph). `houki-egov-mcp` is the exception on both points: it keeps the SDK's low-level `Server` so that its error JSON stays on the houki-hub family contract, so its `inputSchema` carries no `$schema` (plain JSON Schema, unchanged from v0.3.x), and from v0.5.3 a type, required-field or enum violation is rejected before the handler runs as `{ "code": "INVALID_ARGUMENT", "detail": { "issues": [...] } }` with `isError: true` rather than as `Input validation error: ...`; an argument the schema does not declare is passed through. As long as you pass tool names and arguments as declared, nothing changes for `npx` users. Some servers raised their Node requirement in the same release — see the Node.js requirement above. w3c-mcp v0.2.0 removed `get_spec_dependencies`, deprecated since 0.1.9 (upstream `web-specs` never exposed dependency data, so it only ever returned empty arrays; use `get_w3c_spec`).
