...
- **pdf-trust（報告書に何を書けるか）**: `pdf-verify-mcp` の変更 2 件は、判定ではなく**書ける内容**に効きます。**v0.15.1** から、適合の判定を出した veraPDF の版が記録されます（`authoritativeValidation.version`）。規則の数（「146 / 146」など）は**同じビルドの実行どうしでしか比べられない**ので、それ以前の報告書は自分の数字を誰が出したのか言えません。**v0.15.2** では `verify_integrity` の説明を直しました —— 返ってきた `revisions` は**全履歴とは限らず**、短くなる 2 つの場合（元版に届く前にチェーンが終わる／最後の `startxref` が読めるセクションを指していない）は `notes` にしか出ません。真偽のフィールドは無く、「歩き切れた」と言う note もありません（完全性はその 2 文が**無いこと**でしか表せない）。どちらも旧版の判定が誤っていたという話ではなく、**報告書に何を書けるか**が変わります。
